		...
		<div class="col-md-9">
			<div class="card">
				<div class="card-header">
					<h4 class="card-title">{{ $messages.labelCollectMoneyBankEditPage() }}</h4>
				</div>
				<div class="card-body">
                    ...
                    <div class="form-group">
						...
						<div v-if="v$.bankAccountName.$error">
                    ...
                    <div class="form-group">
						<label>{{ $messages.fieldLabelExchangeRate() }}</label>
	...
	methods: {
		...
		async getDetail() {
			const [
				error,
				result,
			] = await this.$service.bankService.getCollectMoneyBankDetail(
				this.recordId
			);
			...
			if (error) {
				...
				this.type = result.content.cmbtType;
			}
		},
		async save() {
			this.v$.$touch();
...
let loader = this.$loading.show({
			...
			const [
				error,
				result,
			] = await this.$service.bankService.updateCollectMoneyBankDetail(record);
