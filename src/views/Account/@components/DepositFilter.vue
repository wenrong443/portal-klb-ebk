			...
			<div class="card">
				...
				<div class="card-body" v-if="isFilter">
					<div class="form-group">
						...
						<input type="email" class="form-control" v-model="cdrtUtUsername" />
					</div>
					<div class="form-group">
						<label for="inputState">{{ $messages.labelMenuCollectMoneyBankList() }}</label>
						<select class="form-control" v-model="cdrtCmbtId">
							<option v-for="typeOption in cmbtRecords" v-bind:value="typeOption.cmbtId"
								v-bind:key="typeOption.cmbtId">
								{{ typeOption.cmbtBankName }} <span v-if="typeOption.cmbtAccountName">-</span>
								{{ typeOption.cmbtAccountName }}
							</option>
						</select>
					</div>
					<div class="form-group">
						<label for="inputState">{{ $messages.labelFilteringSortStatus() }}</label>
						<select class="form-control" v-model="cdrtStatus">
...
export default {
	...
	computed: {
		...
		sortByOptions() {
			return [
				...
				{
					id: "cdrtCmbtId",
					value: this.$messages.labelMenuCollectMoneyBankList(),
				},
				{
					id: "cdrtStatus",
	...
	async created() {
		...
		if (filter) {
			...
			this.cdrtCmbtId = formattedFilter.cdrtCmbtId;
			this.cdrtStatus = formattedFilter.cdrtStatus;
			this.cdrtUtUsername = formattedFilter.cdrtUtUsername;
	...
	methods: {
		async getCollectMoneyBankList() {
			const [error, result] =
				await this.$service.collectMoneyBankService.getAdminAll({});
			if (error) {
			} else {
				this.cmbtRecords = [
					{
						cmbtId: -99,
						cmbtBankName: this.$messages.fieldLabelAll(),
						cmbtAccountName: "",
					},
				];
				this.cmbtRecords = this.cmbtRecords.concat.apply(
					this.cmbtRecords,
					result.content
				);
			}
		...
		resetFilter() {
			sessionStorage.removeItem("ACCOUNT_LISTING_FILTER_LIST");
