<template>
	<div class="row">
		...
		<div class="col-4">
			<div v-if="showBatchActions && $claims.canApproveWithdraw && $claims.canRejectWithdraw">
				<button class="btn btn-danger btn-round" @click="rejectOrders">{{ $messages.labelReject() }}
					({{ selectedItemsLength }})</button>
				<button class="btn btn-info btn-round" @click="approveOrders">{{ $messages.labelApprove() }}
					({{ selectedItemsLength }})</button>
			</div>
		</div>
		<div class="col-4">
			<div class="row">
				<div class="col-6">
					<button class="btn btn-info btn-round float-right"
						@click="getAll">{{ $messages.fieldLabelRefresh() }}
	...
	<div class="overflow-auto">
		<div class="paper-table table table-striped table-bordered">
			...
			<span class="paper-table--header" style="width: 20% !important">{{ $messages.fieldLabelBankInfo() }}</span>
			<span class="paper-table--header">{{ $messages.labelAmount() }}
				({{currency}})</span>
			...
			<div class="paper-table-body">
				<div class="paper-table-rows" v-for="(record, index) in records" v-bind:key="record.id">
					<withdraw-listing-row :record="record" @refresh="getAll()" :selectedItems="selectedItems"
						:languageOptions="languageOptions" :currentIndex="index" :currentPage="currentPage"
						:rowPerPage="rowPerPage" @selected="toggleSelected" />
				</div>
			</div>
		</div>
	</div>
	...
	<div id="myModal" class="modal" v-if="showModal" @focusout="closeDialog" tabindex="0">
		<div class="modal-content">
			<div class="modal-header">
				<span @click="closeDialog" class="close">&times;</span>
				<h6>{{ $messages.labelWithdrawDetailPage() }} #{{modalOrderNo}}
				</h6>
			</div>
			<div class="modal-body">
				<withdraw-detail-page :recordId="modalId" @refresh="getAll">
...
<script>
import WithdrawListingRow from "./WithdrawListingRow";
...
import WithdrawDetailPage from "../WithdrawDetailPage.vue";
import json from "@/infrastructure/json/config.json";

export default {
	name: "ReferrerListing",
	mixins: [pageBehaviour, RecordMixin, listingBehaviour],
	components: { WithdrawListingRow, WithdrawDetailPage },
	props: {
        roc: { type: Number },
		cwrtStatus: { type: Number },
		cwrtUbtId: { type: Number },
		cwrtUtLevelId: { type: Number },
		cwrtUtUsername: { type: String },
			...
			set: function (value) {
				...
				if (value) {
					this.records.forEach((record) => {
						this.selectedItems.push(record.cwrtId);
					});
				}
			},
		},
	...
	methods: {
		...
		async getAll(filter) {
			...
			sessionStorage.setItem(
				"WITHDRAW_LISTING_FILTER_LIST",
				JSON.stringify(selectedFilter)
			);
			const [error, result] = await this.$service.accountService.getWithdrawAll(
				selectedFilter
			);
			if (error) {
				...
				this.records = result.content.map((d) => {
					const memberships = JSON.parse(d.cwrtMtName);
					const userBanks = JSON.parse(d.cwrtUbtName);
					return {
						memberships,
						userBanks,
						...d,
		...
		async rejectOrders() {
			const { value } = await this.$swal({
				...
				inputValidator: async (value) => {
					...
					this.setLoading();
					const [error, result] =
						await this.$service.accountService.batchWithdraw(
							this.selectedItems,
							0,
							value
						);
		...
		async approveOrders() {
			...
			const [error, result] = await this.$service.accountService.batchWithdraw(
				this.selectedItems,
				1,
				"approved"
			);
