	...
	<div class="paper-table table table-striped table-bordered">
		...
		<div class="paper-table-body">
			<div class="paper-table-rows" v-for="(record, index) in records" v-bind:key="record.id">
				<user-manual-fund-history-listing-row :record="record" @refresh="getAll()" :currentIndex="index" :currentPage="currentPage"
...
<script>
import UserManualFundHistoryListingRow from "./UserManualFundHistoryListingRow";
...
export default {
	...
	components: { UserManualFundHistoryListingRow },
	props: {
	...
	methods: {
		async getAll() {
			const [error, result] =
				await this.$service.accountService.getManualFundAllForUserDetails(
					this.userId,
					this.filterCriteria
				);
			if (error) {
