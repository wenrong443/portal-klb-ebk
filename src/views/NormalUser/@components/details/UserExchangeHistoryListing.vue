	...
	<div class="paper-table table table-striped table-bordered">
		...
		<div class="paper-table-body">
			<div class="paper-table-rows" v-for="(record, index) in records" v-bind:key="record.id">
				<user-exchange-history-listing-row :record="record" @refresh="getAll()" :currentIndex="index" :currentPage="currentPage"
...
<script>
import UserExchangeHistoryListingRow from "./UserExchangeHistoryListingRow";
...
export default {
	...
	components: { UserExchangeHistoryListingRow },
	props: {
	...
	methods: {
		async getAll() {
			const [error, result] = await this.$service.mineProductService.getExchangeHistory2(
                this.userId,
				this.filterCriteria
			);
			if (error) {
