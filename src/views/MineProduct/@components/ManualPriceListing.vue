<template>
	<div class="paper-table table table-striped table-bordered">
		<span class="paper-table--header">#</span>
		<span class="paper-table--header">{{ $messages.fieldLabelPrice() }}</span>
		<span class="paper-table--header">{{ $messages.fieldLabelRedPacketStartDate() }}</span>
		<span class="paper-table--header">{{ $messages.fieldLabelRedPacketEndDate() }}</span>
		<span class="paper-table--header">{{ $messages.fieldLabelProcessedBy() }}</span>
		<span class="paper-table--header">{{ $messages.fieldLabelProcessedAt() }}</span>
		<div class="paper-table-body">
			<div class="paper-table-rows" v-for="(record, index) in records" v-bind:key="record.id">
				<manual-price-listing-row :record="record" @refresh="getAll()" :currentIndex="index" :currentPage="currentPage"
					:rowPerPage="rowPerPage" />
			</div>
		</div>
	</div>
	<div v-if="hasRecords" class="no-record">
		<div class="no-record--title">{{ $messages.labelNoRecord() }}</div>
	</div>
	<pagination-control :length="totalPage" v-model="currentPage" @rowPerPage="updateRowPerPage"></pagination-control>
</template>

<script>
import ManualPriceListingRow from "./ManualPriceListingRow";
import pageBehaviour from "seedwork/mixins/pageBehaviour";
import listingBehaviour from "seedwork/mixins/listingBehaviour";

export default {
	name: "AdminListing",
	mixins: [pageBehaviour, listingBehaviour],
	components: { ManualPriceListingRow },
	props: {
        recordId: { type: String, require: true },
    },
	data: () => ({}),
	async created() {
		this.getAll();
	},
	computed: {
		filterCriteria() {
			return {
				pageNumber: this.currentPage,
				rowPerPage: this.rowPerPage,
			};
		},
	},
	methods: {
		async getAll() {
			const [error, result] =
				await this.$service.mineProductService.getManualPrice(
					this.filterCriteria, this.recordId
				);
			if (error) {
			} else {
				this.records = result.content;
				this.totalPage = result.paginationInfo.totalPages;
			}
		},
	},
	watch: {
		filterCriteria() {
			if (this.filterCriteria !== null) {
				this.getAll();
			}
		},
	},
};
</script>

<style lang="scss" scoped></style>
