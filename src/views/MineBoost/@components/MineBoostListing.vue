<template>
  <div class="paper-table table table-striped table-bordered">
    ...
    <span class="paper-table--header">{{ $messages.fieldLabelReduceMinePercent() }}</span>
    <span class="paper-table--header">{{ $messages.fieldLabelBoostMinePercent() }}</span>
    <span class="paper-table--header">{{ $messages.fieldLabelBoostTarget() }}</span>
    <span class="paper-table--header">{{ $messages.fieldLabelEquivalentUSDT() }}</span>
    ...
    <span class="paper-table--header disabled-sorting text-right" v-if="$claims.canEditMineBoost">{{ $messages.labelAction() }}</span>
...
export default {
  ...
  methods: {
    async getAll() {
      const [error, result] = await this.$service.mineBoostService.getAll(
        this.filterCriteria
      );
      if (error) {
      } else {
