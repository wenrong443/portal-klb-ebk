		...
		<div class="col-md-9">
			<div class="card">
				<div class="card-header">
					<h4 class="card-title">
						{{ $messages.labelMineBoostEditPage() }}</h4>
				</div>
				<div class="card-body">
...
export default {
	...
	methods: {
		...
		async getDetail() {
			const [error, result] = await this.$service.mineBoostService.getDetail(
				this.recordId
			);
		...
		async save() {
			...
			const [error, result] = await this.$service.mineBoostService.update(
				record
			);
			loader.hide();
			if (error) {
