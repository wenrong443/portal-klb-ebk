		...
		<div class="col-md-9">
			<div class="card">
				<div class="card-header">
					<h4 class="card-title">{{ $messages.labelMembershipAddPage() }}</h4>
				</div>
				<div class="card-body">
					...
					<div class="form-group">
						...
						<input type="number" class="form-control" :placeholder="$messages.fieldLabelEnter() + $messages.fieldLabelLevel()"
							v-model="mtLevel" />
						<div v-if="v$.mtLevel.$error">
							<div v-if="v$.mtLevel.required" class="error">
								{{ $messages.fieldLabelLevel() }}{{ $messages.fieldLabelIsRequired() }}
							</div>
							<div v-if="v$.mtLevel.minValue" class="error">{{ $messages.fieldLabelMin() }} 2</div>
						</div>
					</div>
					<div class="form-group">
						...
						<input type="number" class="form-control" :placeholder="$messages.fieldLabelEnter() + $messages.fieldLabelPrice()"
							v-model="mtPrice" />
						<div v-if="v$.mtPrice.$error">
							<div v-if="v$.mtPrice.required" class="error">
								{{ $messages.fieldLabelPrice() }}{{ $messages.fieldLabelIsRequired() }}
							</div>
							<div v-if="v$.mtPrice.minValue" class="error">{{ $messages.fieldLabelMin() }} 1</div>
						</div>
					</div>
            ...
            <div v-if="v$.mtRewardPerTask.$error">
              ...
              <div v-if="v$.mtRewardPerTask.minValue" class="error">
                {{ $messages.fieldLabelMin() }} 1
					...
					<div class="form-group">
						<cave-image-upload :title="$messages.fieldLabelImageBanner()" v-model="banner"></cave-image-upload>
					</div>
					...
					<cave-switch-checkbox v-model="mtStatus" :text="$messages.labelStatus()"></cave-switch-checkbox>
				</div>
...
export default {
	...
	methods: {
		...
		async save() {
			...
			const [error, result] = await this.$service.membershipService.add(
				record
			);
