<template>
	<div class="paper-table table table-striped table-bordered">
		...
		<span class="paper-table--header">{{ $messages.fieldLabelMaxAmount() }} ({{currency}})</span>
		<span class="paper-table--header">{{ $messages.fieldLabelTotalIssuing() }}</span>
		<span class="paper-table--header">{{ $messages.fieldLabelInvestmentCycle() }}</span>
		...
		<div class="paper-table-body">
			<div class="paper-table-rows" v-for="(record, index) in formattedRecords" v-bind:key="record.id">
				<investment-fund-listing-row :record="record" :currentIndex="index" :currentPage="currentPage" :rowPerPage="rowPerPage"
					@refresh="getAll" />
			</div>
		</div>
	</div>
...
<script>
import InvestmentFundListingRow from "./InvestmentFundListingRow";
...
export default {
	...
	components: { InvestmentFundListingRow },
	props: {
		giptPayoutMode: { type: String },
		giptStatus: { type: Number },
		sortBy: { type: String },
	...
	computed: {
		filterCriteria() {
			return {
				giptPayoutMode: this.giptPayoutMode,
				giptStatus: this.giptStatus,
				paginationDAO: {
		...
		formattedRecords() {
			...
			const items = [];
			this.records.map((d) => {
				const title = JSON.parse(d.giptDisplayTitle);
				title.forEach((i) => {
					const obj = {
						language: i.ltId,
						title: i.value,
	...
	methods: {
		async getAll() {
			const [error, result] = await this.$service.investmentService.getAll(
				this.filterCriteria
			);
			if (error) {
			} else {
