			...
			<div class="card">
				...
				<div class="card-body" v-if="isFilter">
                    <div class="form-group">
						...
						<input type="email" class="form-control" v-model="contactNumber" />
					</div>
					<div class="form-group">
						<label for="inputState">{{ $messages.labelStatus() }}</label>
						<select class="form-control" v-model="grttStatus">
...
<script>
import { mixin as RecordMixin } from "../ReviewTaskTaken";
export default {
	components: {},
	mixins: [RecordMixin],
	data: () => ({
	...
	methods: {
		...
		resetFilter() {
			this.contactNumber = "";
            this.grttStatus = -99
			this.sortBy = "grttUpdatedDatetime";
