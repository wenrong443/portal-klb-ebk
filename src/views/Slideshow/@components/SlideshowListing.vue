...
<script>
import SlideshowListingRow from "./SlideshowListingRow";
...
export default {
  ...
  components: { SlideshowListingRow },
  props: {
  },
  data: () => ({}),
  async created() {
  ...
  methods: {
    async getAll() {
      const [error, result] = await this.$service.slideshowService.getAll(
        this.filterCriteria
      );
      if (error) {
      } else {
