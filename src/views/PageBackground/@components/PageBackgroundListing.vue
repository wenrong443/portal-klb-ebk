<template>
  <div class="paper-table table table-striped table-bordered">
    ...
    <span class="paper-table--header">{{ $messages.fieldLabelImage() }}</span>
    ...
    <span class="paper-table--header disabled-sorting text-right" v-if="$claims.canEditSlideshow">{{ $messages.labelAction() }}</span>
...
<script>
import PageBackgroundListingRow from "./PageBackgroundListingRow";
import pageBehaviour from "seedwork/mixins/pageBehaviour";
import listingBehaviour from "seedwork/mixins/listingBehaviour";

export default {
  name: "PageBackgroundListing",
  mixins: [pageBehaviour, listingBehaviour],
  components: { PageBackgroundListingRow },
  props: {
  },
  data: () => ({}),
  async created() {
  ...
  methods: {
    async getAll() {
      const [error, result] = await this.$service.pageBackgroundService.getAll(
        this.filterCriteria
      );
      if (error) {
      } else {
