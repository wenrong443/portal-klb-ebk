<template>
	<td>{{ rowIndex }}</td>
	<td class="cave-table-row--center text-xs">{{ record.pbtUiLink }}</td>
	<td class="cave-table-row--center">
		<img @click="openImageInNewTab(record.pbtImage)" class="image-picker"
			:src="record.pbtImage" alt=""/>
	</td>
    <td class="cave-table-row--center">{{ record.pbtRemark }}</td>
...
export default {
	...
	computed: {
		formattedStatusClass() {
			return this.record.pbtStatus != null
				? STATUS_CLASS[this.record.pbtStatus]
				: "";
		},
		formattedStatusName() {
			return this.record.pbtStatus != null
				? this.STATUS[this.record.pbtStatus]
				: "-";
		},
	},
	async created() {
	...
	methods: {
		...
		goToEdit() {
			this.$router.push({
				name: "PageBackgroundEditPage",
				params: { recordId: this.record.pbtId },
			});
		},
		async goToDelete() {
			...
			if (value.isConfirmed) {
				...
				const [error, result] = await this.$service.pageBackgroundService.delete(
					this.record.pbtId
				);
				loader.hide();
				if (error) {
