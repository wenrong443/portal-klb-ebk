<template>
	<div class="paper-table table table-striped table-bordered">
		...
		<span class="paper-table--header">{{ $messages.fieldLabelRedPacketMaxPerUser() }}</span>
        ...
        <span class="paper-table--header">{{ $messages.labelDistributePax() }}</span>
        <span class="paper-table--header">{{ $messages.labelDistributeUSDT() }}</span>
...
export default {
	...
	methods: {
		async getAll() {
			const [error, result] = await this.$service.redPacketService.getAll();
			if (error) {
			} else {
