		...
		<div class="col-md-9">
			<div class="card">
				...
				<div class="card-body">
					...
					<red-packet-sub ref="redPacketSub" :productList="productList"></red-packet-sub>
					<div v-if="isShowNestedListError">
						<div class="error">
							{{ $messages.fieldLabelRedPacketSub() }}{{ $messages.fieldLabelIsRequired() }}
						</div>
					</div>
					<cave-image-upload v-model="image" :title="$messages.fieldLabelRedPacketImage()"></cave-image-upload>
                    ...
                    <cave-image-upload v-model="animationImage" :title="$messages.fieldLabelRedPacketAnimationImage()"></cave-image-upload>
...
export default {
	...
	methods: {
		...
		async save() {
			...
			this.setLoading();
			const [error, result] = await this.$service.redPacketService.add(record);
			if (error) {
				this.showError(error);
			} else {
