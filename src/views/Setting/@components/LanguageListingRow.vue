	...
	<td class="text-right">
		<div class="dropdown">
			...
			<div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
				<a class="dropdown-item" :href="formattedDownloadLink" target="_blank" download>{{ $messages.fieldLabelDownloadLanguagePack() }}</a>
...
export default {
	...
	computed: {
		...
		formattedStatusName() {
			return this.record.ltStatus != null
				? this.STATUS[this.record.ltStatus]
				: "-";
		},
	...
	methods: {
		...
		async deleteRecord() {
			...
			if (value.isConfirmed) {
				...
				const [error, result] =
					await this.$service.settingService.deleteLanguage(
						this.record.ltId
					);
                    loader.hide();
				if (error) {
