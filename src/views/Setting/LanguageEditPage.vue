		...
		<div class="col-md-9">
			<div class="card">
				<div class="card-header">
					<h4 class="card-title">{{ $messages.labelLanguageEditPage() }}</h4>
				</div>
				<div class="card-body">
					...
					<div class="form-group">
						...
						<input type="email" class="form-control" :placeholder="$messages.fieldLabelEnter() + $messages.fieldLabelCode()" v-model="ltFileName" disabled />
...
export default {
	...
	methods: {
		async getDetail() {
			const [error, result] = await this.$service.settingService.getLanguage(
				this.recordId
			);
...
let loader = this.$loading.show({
			...
			const [error, result] = await this.$service.settingService.updateLanguage(
				record
			);
            loader.hide();
